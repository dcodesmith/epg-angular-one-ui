...
export default function HomeController($scope, ChannelService, ProgrammeService) {
  ...
  function parseProgrammes(programme, index, programmes) {
	...
	// function setProgrammeDates(programmes) {
	// 	let programmeDates = programmes.map(function(programme) {
	// 		return new Date(programme.date);
	// 	});
	// 	let startDate = new Date(Math.max.apply(null, programmeDates));
	// 	let endDate = new Date(Math.min.apply(null, programmeDates));
	//
	// 	vm.programme = {
	// 		startDate: startDate,
	// 		endDate: endDate
